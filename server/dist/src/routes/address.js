...
addressRouter.get('/address/:addr/assets', validateParams(AddressParamsSchema), async (req, res) => {
    ...
    try {
        ...
        const assets = Array.isArray(assetsFirst?.asset_list)
            ? assetsFirst.asset_list
        ...
        try {
            req.log?.info({ address: addr, assetsCount: assets.length, utxosCount: utxos.length }, 'Address data fetched from Koios');
        }
        catch {
        ...
        req.log?.error({ err }, 'Failed to fetch assets from Koios');
