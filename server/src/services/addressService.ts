...
export class AddressService {
  static async saveAddress(userId: string, address: string, provider: string): Promise<SavedAddress> {
    try {
  ...
  static async getAddressesByUserAndProvider(userId: string, provider: string): Promise<SavedAddress[]> {
    try {
      const addresses = await dbAll(
  ...
  static async removeAllAddresses(userId: string): Promise<number> {
    try {
      const result = await dbRun(
        'DELETE FROM saved_addresses WHERE user_id = ?',
        [userId]
      );
